...
from config import OLLAMA_MODEL, TOP_K, CACHE_ENABLED
...
def recommend():
    ...
    # 确保系统已初始化
    model_name = request.args.get('model', OLLAMA_MODEL)
    with system_lock:
    ...
    try:
        ...
        recommendations = []
        movie_id_map = {row['item_id']: row for _, row in movies.iterrows()}
...
if __name__ == '__main__':
    # 初始化系统（关键修复！）
    if system_components is None:
        print(f"=================== 初始化系统 (模型: {OLLAMA_MODEL}) ===================")
        system_components = init_system(OLLAMA_MODEL)
        if system_components is None:
            print("❌ 系统初始化失败，退出运行")
            exit(1)
        print("✅ 系统初始化成功！")
