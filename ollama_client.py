...
class OllamaClient:
    ...
    def generate(self, prompt, max_tokens=200, temperature=0.7):
        ...
        try:
            ...
            print(f"LLM生成完成，耗时: {time.time() - start_time:.2f}秒")
            return result.get("response", "")
            
        except requests.exceptions.Timeout:
            print("❌ Ollama调用超时 - 尝试使用更小的模型或增加超时时间")
            return ""
        except Exception as e:
    ...
    def get_recommendations(self, user_id, build_prompt_func, users, user_preferences_func):
        """获取用户推荐，修复参数传递"""
        # 注意参数顺序: build_prompt_func(user_id, users, user_preferences_func)
        ...
        return recommendations[:5]  # 限制返回数量
