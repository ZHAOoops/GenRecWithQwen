...
def run_evaluation(model_name="qwen2:7b", sample_size=20, k=10):
    ...
    try:
        ...
        # 补充导入build_prompt（修复原代码中未正确引用的问题）
        from data_preparation import build_prompt
        
        # 定义召回函数（适配merge_recalls的参数要求）
        def recall_func(user_id, top_k=10):
            ...
            return merge_recalls(
                ...
                ratings_df=ratings,
                build_prompt_func=build_prompt,  # 修复原代码中的引用错误
                users=users,
        ...
        def rank_func(user_id, candidates):
            return ranker.rank(
                ...
                users_df=users,
                ratings_df=ratings,
                return_scores=True  # 明确要求返回分数，适配评估逻辑
            )
        
        # 筛选有足够交互的用户（确保评估有效性）
        valid_users = [
            uid for uid in ratings['user_id'].unique()
            if len(ratings[ratings['user_id'] == uid]) >= 10  # 至少有10条评分记录
        ]
